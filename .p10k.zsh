...
() {
  ...
  function prompt_my_cpu_temp() {
    ...
    if (( cpu_temp >= 65 )); then
      ...
      p10k segment -s WARM -f yellow -i "☁️" -t "${cpu_temp}°C"
    else
      p10k segment -s COLD -f blue -i "❄️" -t "${cpu_temp}°C"
    fi
  }
  ...
  function prompt_my_wifi_signal() {
    ...
    if (( wifi_signal >= 70 )); then
      p10k segment -s HIGH -f red -i "📶" -t "${wifi_signal}"
    elif (( wifi_signal >0 )); then
      p10k segment -s LOW -f yellow -i "📶" -t "${wifi_signal}"
    else
      p10k segment -s NONE -f white -i "✈️" -t "${wifi_signal}"
    fi
  }
